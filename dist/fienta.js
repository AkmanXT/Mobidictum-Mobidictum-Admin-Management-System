...
class FientaClient {
    ...
    async updateExistingCodes(records, percent) {
        const page = this.requirePage();
        const discountsUrl = this.config.discountsUrl || `${this.config.fientaBaseUrl}/my/events/discounts`;
        logger_1.logger.info({ count: records.length, percent }, 'Updating existing discount codes to target percent');
        ...
        for (const record of records) {
            ...
            try {
                ...
                if (!(await link.count())) {
                    logger_1.logger.warn({ code }, 'Code not found for update');
                    continue;
                }
                await link.click();
                await page.waitForLoadState('networkidle');
                ...
                logger_1.logger.info({ code }, 'Updated');
            }
            catch (err) {
                logger_1.logger.error({ err, code }, 'Failed to update code');
            }
        }
    }
    async start() {
        ...
        const storageExists = !!storagePath && fs_1.default.existsSync(storagePath);
    ...
    async login() {
        ...
        logger_1.logger.info('Logging in to Fienta...');
        ...
        if (loginFormVisible) {
            if (this.config.manualLogin) {
                ...
                logger_1.logger.info({ timeoutMs: this.config.loginTimeoutMs }, 'Waiting for manual login...');
            ...
            if (this.config.storageStatePath) {
                const dir = path_1.default.dirname(this.config.storageStatePath);
            ...
            logger_1.logger.info('Already logged in, continuing.');
        }
    }
    // Placeholder: implement according to actual Fienta UI for codes management
    async createOrCustomizeCodes(records) {
        const page = this.requirePage();
        logger_1.logger.info({ count: records.length }, 'Creating new discount codes');
        ...
        for (const record of records) {
            if (this.config.dryRun) {
                logger_1.logger.info({ record }, 'DRY RUN - would create code');
                continue;
            }
            try {
                const code = String(record.code || '').trim();
                if (!code) {
                    logger_1.logger.warn({ record }, 'Skipping record without code');
                    continue;
                }
                ...
                if (await addHref.count()) {
                    try {
                        await addHref.scrollIntoViewIfNeeded();
                        await addHref.click();
                        await page.waitForLoadState('networkidle');
                        clicked = true;
                    }
                ...
                await codeField.evaluate((el) => el.blur());
                ...
                if (await orderInput.count()) {
                    ...
                    await orderInput.evaluate((el) => el.blur());
                }
                ...
                if (await ticketInput.count()) {
                    ...
                    await ticketInput.evaluate((el) => el.blur());
                }
                ...
                logger_1.logger.info({ code }, 'Created discount code');
            }
            catch (err) {
                logger_1.logger.error({ err, record }, 'Failed to create code');
            }
        }
    }
    async renameExistingCodes(pairs) {
        const page = this.requirePage();
        logger_1.logger.info({ count: pairs.length }, 'Renaming existing codes');
        const discountsUrl = this.config.discountsUrl || `${this.config.fientaBaseUrl}/my/events/discounts`;
        await page.goto(discountsUrl);
        await page.waitForLoadState('networkidle');
        for (const pair of pairs) {
            if (!pair.old || !pair.new)
                continue;
            if (this.config.dryRun) {
                logger_1.logger.info({ pair }, 'DRY RUN - would rename code');
                continue;
            }
            try {
                async function searchAndOpenByCode(code) {
                    try {
                        ...
                        const clicked = await rowEl.evaluate((node) => {
                            const container = node.closest('[class*="row"], li, tr, div');
                            if (!container)
                                return false;
                            const linkInside = container.querySelector('a[href*="/discounts/"]');
                            if (linkInside) {
                                linkInside.click();
                                return true;
                            }
                ...
                if (!opened) {
                    logger_1.logger.warn({ pair }, 'Old code not found, trying new code');
                    try {
                ...
                if (!opened) {
                    logger_1.logger.warn({ pair }, 'Code not found in list');
                    continue;
                }
                ...
                if (this.config.limitPerOrder !== undefined) {
                    ...
                    if (await orderInput.count()) {
                        ...
                        logger_1.logger.warn({ pair }, 'Order limit input not found');
                    }
                }
                if (this.config.limitPerTicket !== undefined) {
                    ...
                    if (await ticketInput.count()) {
                        ...
                        logger_1.logger.warn({ pair }, 'Ticket limit input not found');
                    }
                }
                ...
                logger_1.logger.info({ pair }, 'Renamed');
                ...
                logger_1.logger.error({ err, pair }, 'Failed to rename');
            }
        }
    }
}
exports.FientaClient = FientaClient;
